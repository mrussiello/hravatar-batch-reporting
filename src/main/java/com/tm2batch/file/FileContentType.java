...
public enum FileContentType
{
    ...
    public static FileContentType getCorrectedFileContentType( FileContentType currentFct , String vfmt, String afmt )
    {
        //boolean hasAudio = afmt != null && !afmt.isEmpty() && !afmt.equalsIgnoreCase( "null" );

        boolean hasVideo = vfmt != null && !vfmt.isEmpty() && !vfmt.equalsIgnoreCase( "null" );

        if( currentFct.equals( FileContentType.AUDIO_MP4)  && hasVideo )
            return FileContentType.VIDEO_MP4;

        if( currentFct.equals( FileContentType.AUDIO_WEBM)  && hasVideo )
            return FileContentType.VIDEO_WEBM;

        if( currentFct.equals( FileContentType.VIDEO_MP4)  && !hasVideo )
            return FileContentType.AUDIO_MP4;

        if( currentFct.equals( FileContentType.VIDEO_WEBM)  && !hasVideo )
            return FileContentType.AUDIO_WEBM;
    ...
    {
            if( equals( FileContentType.FONT_EOT ) )
                return "embedded-opentype";

            else if( equals( FileContentType.FONT_OTF ) )
                return "opentype";

            else if( equals( FileContentType.FONT_TTF ) )
                return "truetype";

            else if( equals( FileContentType.FONT_WOFF ) )
                return "woff";

            else if( equals( FileContentType.FONT_SVG ) )
                return "svg";
