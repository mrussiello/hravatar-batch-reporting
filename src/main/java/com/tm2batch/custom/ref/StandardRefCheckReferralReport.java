...
public class StandardRefCheckReferralReport extends BaseExecutableReport implements ExecutableReport {
    ...
    public void validateBatchReportForExecution() throws Exception
    {
        ...
        {
            LogService.logIt( "StandardRefCheckReferralReport.validateBatchReportForExecution() BatchReport id=" + batchReport.getBatchReportId() + " does not look back at least one day or one hour or month or year. daysBack=" + batchReport.getDaysBack() + ", hoursBack=" + batchReport.getHoursBack() + ", setting to 1 day back." );
