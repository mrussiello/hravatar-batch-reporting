...
import com.tm2batch.custom.leaderstyle.LeaderStyleGroupDataSet;
import com.tm2batch.custom.leaderstyle.LeaderStyleReportUtils;
import com.tm2batch.custom.leaderstyle.LeaderStyleResult;
import com.tm2batch.custom.leaderstyle.LeaderStyleStyleNameValueComparator;
import com.tm2batch.custom.leaderstyle.LeaderStyleUserNameComparator;
 ...
 * @author Mike
 */
public abstract class BaseLeaderStyleGroupReportTemplate extends LeaderStyleGroupReportSettings implements ReportTemplate
{
    ...
    LeaderStyleReportUtils leaderStyleReportUtils = null;
    ...
    public synchronized void initFonts() throws Exception
    {
        ...
        if( logoUrl!=null && !logoUrl.isBlank() )
        {
            ...
            {
                LogService.logIt( "BaseLeaderStyleGroupReportTemplate.initFonts() AAA.1 LogoURL appears to be invalid format. Ignoring. OrgId=" + reportData.o.getOrgId() + ", logo=" + logoUrl );
                logoUrl=null;
            }
        }
        ...
        catch( Exception e )
        {
            ...
            {
                LogService.logIt( "BaseLeaderStyleGroupReportTemplate.initFonts() BBB.1 NONFATAL error getting custLogo. Will try http instead of https. logo=" + logoUrl );
                ...
                catch( IOException ee )
                {
                    ...
                    LogService.logIt( "BaseLeaderStyleGroupReportTemplate.initFonts() BBB.3 NONFATAL error getting custLogo using http. OrgId=" + orgId + ". Will use null. logo=" + logoUrl + ", logo2=" + logo2 );
                }
                catch( Exception ee )
                {
                    custLogo=null;
                    int orgId=reportData==null || reportData.getOrg()==null ? 0 : reportData.getOrg().getOrgId();
                    LogService.logIt( ee, "BaseLeaderStyleGroupReportTemplate.initFonts() BBB.6 NONFATAL error getting custLogo using http. OrgId=" + orgId + ". Will use null. logo=" + logoUrl + ", logo2=" + logo2 );
                }
            }

            else
            {
                LogService.logIt( "BaseLeaderStyleGroupReportTemplate.initFonts() CCC.1 NONFATAL error getting custLogo. Will use null. logo= " + logoUrl + ", Exception=" + e.toString() );
            }
        }
    }
    ...
    public void init( ReportData rd ) throws Exception
    {
        ...
        leaderStyleDataSet = (LeaderStyleGroupDataSet) rd.objArray[0];
        ...
        LeaderStyleHeaderFooter hdr = new LeaderStyleHeaderFooter( document, rd.getLocale(), rd.getReportName(), reportData, this, custLogo, groupReportFooterImageUrls );
    ...
    public synchronized void specialInit()
    {
        if( leaderStyleReportUtils==null )
        {
            ...
            if( bundleToUse==null || bundleToUse.isEmpty() )
            {
                ...
                if( loc.getLanguage().equalsIgnoreCase( "en" ) )
                    bundleToUse = "leaderstylegroup.properties";
                else
                    bundleToUse = "leaderstylegroup_" + loc.getLanguage().toLowerCase() + ".properties";
            }
        ...
        // LogService.logIt( "BaseLeaderStyleGroupReportTemplate.specialInit() groupReportCoverImageUrl=" + groupReportCoverImageUrl );
        ...
        {
            LogService.logIt( e, "BaseLeaderStyleGroupReportTemplate.specialInit() reportId=" + (reportData!=null && reportData.r!=null ? reportData.r.getReportId() : "null") + ", groupReportCoverImageUrl=" + groupReportCoverImageUrl );
        }
    }
    ...
    public void addCoverPage( boolean includeDescriptiveText ) throws Exception
    {
        try
        {
            ...
            LogService.logIt( "BaseLeaderStyleGroupReportTemplate.addCoverPage()  CCC.1 includeCompanyInfo=" + includeCompanyInfo + ", reportCompanyName=" + reportCompanyName + ", compNameForAdmin=" + compNameForAdmin + ", omitCoverImages=" + omitCoverImages );
            ...
            // LogService.logIt( "BaseLeaderStyleGroupReportTemplate.addCoverPageV2() START page dims=" + pageWidth + "," + pageHeight + ", imageDims=" + hraCover.getWidth() + "," + hraCover.getHeight() );
            ...
            if( !omitCoverImages )
            {
                ...
                LogService.logIt( "BaseLeaderStyleGroupReportTemplate.addCoverPageV2() hraCover2=" + (hraCover2==null ? "null" : "not null") );
                if( hraCover2 !=null )
                {
            ...
            // LogService.logIt( "BaseLeaderStyleGroupReportTemplate.addCoverPage()  CCC.2 includePreparedFor=" + includePreparedFor + ", clientLogoInHeader=" + clientLogoInHeader  +", reportData.hasCustLogo()=" + reportData.hasCustLogo() + ", custLogo!=null " + (custLogo!=null));
            ...
            if( coverInfoOk )
            {
                ...
                t.addCell(new Phrase( getSampleReport() ? SampleReportUtils.getCompanyName() : leaderStyleDataSet.getOrg().getName(), cpFont ) );

                if( leaderStyleDataSet.getSuborg()!=null )
                {
                    t.addCell(new Phrase( lmsg( "g.Suborg" ) + ":" , cpFont ) );
                    t.addCell(new Phrase( getSampleReport() ? SampleReportUtils.getRandomSuborg() : leaderStyleDataSet.getSuborg().getName(), cpFont ) );
                }

                if( leaderStyleDataSet.getProduct()!=null )
                {
                    t.addCell(new Phrase( lmsg( "g.Assessment" ) + ":" , cpFont ) );
                    t.addCell(new Phrase( leaderStyleDataSet.getProduct().getName(), cpFont ) );
                }

                if( leaderStyleDataSet.getOrgAutoTest()!=null )
                {
                    t.addCell(new Phrase( lmsg( "g.OrgAutoTest" ) + ":" , cpFont ) );
                    t.addCell(new Phrase( leaderStyleDataSet.getOrgAutoTest().getName(), cpFont ) );
                }

                if( leaderStyleDataSet.getAltIdentifier()!=null && !leaderStyleDataSet.getAltIdentifier().isBlank() )
                {
                    t.addCell(new Phrase( lmsg( "g.AltIdentifier" ) + ":" , cpFont ) );
                    t.addCell(new Phrase( leaderStyleDataSet.getAltIdentifier(), cpFont ) );
                }

                if( leaderStyleDataSet.getCustom1()!=null && !leaderStyleDataSet.getCustom1().isBlank() )
                {
                    String cn = leaderStyleDataSet.getCustom1Name()!=null && !leaderStyleDataSet.getCustom1Name().isBlank() ? leaderStyleDataSet.getCustom1Name() : lmsg( "g.Custom1" );
                    t.addCell(new Phrase( cn + ":" , cpFont ) );
                    t.addCell(new Phrase( leaderStyleDataSet.getCustom1(), cpFont ) );
                }

                if( leaderStyleDataSet.getCustom2()!=null && !leaderStyleDataSet.getCustom2().isBlank() )
                {
                    String cn = leaderStyleDataSet.getCustom2Name()!=null && !leaderStyleDataSet.getCustom2Name().isBlank() ? leaderStyleDataSet.getCustom2Name() : lmsg( "g.Custom2" );
                    t.addCell(new Phrase( cn + ":" , cpFont ) );
                    t.addCell(new Phrase( leaderStyleDataSet.getCustom2(), cpFont ) );
                }

                if( leaderStyleDataSet.getCustom3()!=null && !leaderStyleDataSet.getCustom3().isBlank() )
                {
                    String cn = leaderStyleDataSet.getCustom3Name()!=null && !leaderStyleDataSet.getCustom3Name().isBlank() ? leaderStyleDataSet.getCustom3Name() : lmsg( "g.Custom3" );
                    t.addCell(new Phrase( cn + ":" , cpFont ) );
                    t.addCell(new Phrase( leaderStyleDataSet.getCustom3(), cpFont ) );
                }

                if( includeDates )
                {
                    t.addCell(new Phrase( lmsg( "g.Dates" ) + ":" , cpFont ) );
                    t.addCell(new Phrase( I18nUtils.getFormattedDate(reportData.getLocale(), leaderStyleDataSet.getStartDate(), DateFormat.SHORT) + "-" + I18nUtils.getFormattedDate(reportData.getLocale(), leaderStyleDataSet.getEndDate(), DateFormat.SHORT), cpFont ) );
                }
            }
            ...
            if( includeDescriptiveText )
            {
                ...
                else
                {
                    ...
                    LogService.logIt(  "BaseLeaderStyleGroupReportTemplate.addCoverPage() coverDetailKey=" + coverDetailKey + ", coverDescrip=" + coverDescrip );
                }
        ...
        {
            LogService.logIt( e, "BaseLeaderStyleGroupReportTemplate.addCoverPage()" );
        }
    }
    ...
    public void addReportInfoHeader() throws Exception
    {
        try
        {
            ...
            touter.setTotalWidth( (pageWidth - 2*CT2_MARGIN - 2*CT2_BOX_EXTRAMARGIN)*0.8f );
            ...
            touter.addCell(new Phrase( getSampleReport() ? SampleReportUtils.getCompanyName() : leaderStyleDataSet.getOrg().getName(), cpFont ) );

            if( leaderStyleDataSet.getSuborg()!=null )
            {
                touter.addCell(new Phrase( lmsg( "g.Suborg" ) + ":" , cpFont ) );
                touter.addCell(new Phrase( getSampleReport() ? SampleReportUtils.getRandomSuborg() : leaderStyleDataSet.getSuborg().getName(), cpFont ) );
            }

            if( leaderStyleDataSet.getProduct()!=null )
            {
                touter.addCell(new Phrase( lmsg( "g.Assessment" ) + ":" , cpFont ) );
                touter.addCell(new Phrase( leaderStyleDataSet.getProduct().getName(), cpFont ) );
            }

            if( leaderStyleDataSet.getOrgAutoTest()!=null )
            {
                touter.addCell(new Phrase( lmsg( "g.OrgAutoTest" ) + ":" , cpFont ) );
                touter.addCell(new Phrase( leaderStyleDataSet.getOrgAutoTest().getName(), cpFont ) );
            }

            if( leaderStyleDataSet.getAltIdentifier()!=null && !leaderStyleDataSet.getAltIdentifier().isBlank() )
            {
                touter.addCell(new Phrase( lmsg( "g.AltIdentifier" ) + ":" , cpFont ) );
                touter.addCell(new Phrase( leaderStyleDataSet.getAltIdentifier(), cpFont ) );
            }
            ...
            if( leaderStyleDataSet.getCustom1()!=null && !leaderStyleDataSet.getCustom1().isBlank() )
            {
                ...
                touter.addCell(new Phrase( leaderStyleDataSet.getCustom1(), cpFont ) );
            }
            ...
            if( leaderStyleDataSet.getCustom2()!=null && !leaderStyleDataSet.getCustom2().isBlank() )
            {
                String cn = leaderStyleDataSet.getCustom2Name()!=null && !leaderStyleDataSet.getCustom2Name().isBlank() ? leaderStyleDataSet.getCustom2Name() : lmsg( "g.Custom2" );
                touter.addCell(new Phrase( cn + ":" , cpFont ) );
                touter.addCell(new Phrase( leaderStyleDataSet.getCustom2(), cpFont ) );
            }
            ...
            if( leaderStyleDataSet.getCustom3()!=null && !leaderStyleDataSet.getCustom3().isBlank() )
            {
                String cn = leaderStyleDataSet.getCustom3Name()!=null && !leaderStyleDataSet.getCustom3Name().isBlank() ? leaderStyleDataSet.getCustom3Name() : lmsg( "g.Custom3" );
                touter.addCell(new Phrase( cn + ":" , cpFont ) );
                touter.addCell(new Phrase( leaderStyleDataSet.getCustom3(), cpFont ) );
            }

            if( includeDates )
            {
                touter.addCell(new Phrase( lmsg( "g.Dates" ) + ":" , cpFont ) );
                touter.addCell(new Phrase( I18nUtils.getFormattedDate(reportData.getLocale(), leaderStyleDataSet.getStartDate(), DateFormat.SHORT) + "-" + I18nUtils.getFormattedDate(reportData.getLocale(), leaderStyleDataSet.getEndDate(), DateFormat.SHORT), cpFont ) );
            }

            touter.addCell(new Phrase( lmsg( "g.TotalParticipants" ) + ":" , cpFont ) );
            touter.addCell(new Phrase( Integer.toString(leaderStyleDataSet.getTotalCount()), cpFont ) );
            ...
            if( includeColorGraph )
            {
                ...
                float legendWid = (pageWidth - 2*CT2_MARGIN - 2*CT2_BOX_EXTRAMARGIN)*0.65f;
                ...
                c.setCellEvent( new LeaderStyleGroupPieGraphCellEvent( scoreValMap, scrDigits, ct2Colors, this.leaderStyleReportUtils, reportData.getLocale(), fontLarge.getBaseFont() ) );
                touter.addCell(c);

                float newTHeight = touter.calculateHeights();

                touter.writeSelectedRows(0, -1,(pageWidth - colWid)/2, y, pdfWriter.getDirectContent() );
                ...
                for( int i=0; i<5; i++ )
                {
                    styleStub = LeaderStyleReportUtils.getCompetencyStub(i);
                    data = scoreValMap.get(styleStub );
                    nm = (String) data[0];
                    val = (Float) data[1];
                    count = (Integer) data[2];
                    // LogService.logIt( "BaseLeaderStyleGroupReportTemplate.addReportInfoHeader() i=" + i +", letter=" + styleLetter + ", nm=" + nm + ", val=" + val + ", count=" + count );

                    fnt = new Font(bf, this.FONTSZ);
                    bc = LeaderStyleReportUtils.sliceBaseColors[i];
                    ...
                    c.setBorderWidth( 0 );
                    c.setPadding( 2 );
                    c.setCellEvent(new LeaderStyleGroupLegendCellBackgroundCellEvent(bc) );
                    setRunDirection( c );
                ...
                legendTable.writeSelectedRows(0, -1,(pageWidth-legendWid)/2, y, pdfWriter.getDirectContent() );
        ...
        {
            LogService.logIt( e, "BaseLeaderStyleGroupReportTemplate.addReportInfoHeader()" );
            throw e;
        }
    }
    ...
    */
    public void addScoreTable( int sortTypeId ) throws Exception
    {
        try
        {
            ...
            boolean includeScores = !reportData.getReportRuleAsBoolean("cmptysumoff") && !reportData.getReportRuleAsBoolean("cmptynumoff");

            
            // First, add a table
            PdfPTable t = new PdfPTable( includeScores ? 8 : 3 );
            t.setTotalWidth( pageWidth - 2*CT2_MARGIN - 2*CT2_BOX_EXTRAMARGIN );
            t.setWidths( includeScores ? new float[]{3,3,1,1,1,1,1,1.5f} : new float[]{3,3,7.5f} );
            t.setLockedWidth( true );
            setRunDirection( t );
            ...
            if( includeScores )
            {
                c = new PdfPCell( new Phrase( lmsg_spec(LeaderStyleReportUtils.getCompetencyStub(0)+".shortname"), fontLargeWhite ) );
                c.setBorder( Rectangle.NO_BORDER );
                c.setBorderColor( ct2Colors.scoreBoxBorderColor );
                c.setBorderWidth( scoreBoxBorderWidth );
                c.setPadding( padding );
                ...
                c = new PdfPCell( new Phrase( lmsg_spec(LeaderStyleReportUtils.getCompetencyStub(1)+".shortname"), fontLargeWhite ) );
                c.setBorder( Rectangle.NO_BORDER );
                c.setBorderColor( ct2Colors.scoreBoxBorderColor );
                c.setBorderWidth( scoreBoxBorderWidth );
                c.setPadding( padding );
                ...
                c = new PdfPCell( new Phrase( lmsg_spec(LeaderStyleReportUtils.getCompetencyStub(2)+".shortname"), fontLargeWhite ) );
                c.setBorder( Rectangle.NO_BORDER );
                c.setBorderColor( ct2Colors.scoreBoxBorderColor );
                c.setBorderWidth( scoreBoxBorderWidth );
                c.setPadding( padding );
                ...
                c = new PdfPCell( new Phrase( lmsg_spec(LeaderStyleReportUtils.getCompetencyStub(3)+".shortname"), fontLargeWhite ) );
                c.setBorder( Rectangle.NO_BORDER );
                c.setBorderColor( ct2Colors.scoreBoxBorderColor );
                c.setBorderWidth( scoreBoxBorderWidth );
                c.setPadding( padding );
                ...
                c = new PdfPCell( new Phrase( lmsg_spec(LeaderStyleReportUtils.getCompetencyStub(4)+".shortname"), fontLargeWhite ) );
                c.setBorder( Rectangle.NO_BORDER );
                c.setBorderColor( ct2Colors.scoreBoxBorderColor );
                c.setBorderWidth( scoreBoxBorderWidth );
                c.setPadding( padding );
            ...
            List<LeaderStyleResult> drl = new ArrayList<>();
            drl.addAll(leaderStyleDataSet.getLeaderStyleResultList());
            ...
            // sort by doubletrait
            if( sortTypeId==0 )
                Collections.sort( drl, new LeaderStyleStyleNameValueComparator() );
            else
                Collections.sort( drl, new LeaderStyleUserNameComparator() );
            ...
            for( LeaderStyleResult dr : drl )
            {
                ...
                par = new Paragraph();
                //styleLetter = LeaderStyleReportUtils.getCompetencyStubLetter(dr.getDominantTypeIndex());
                dfont = new Font(bf, FONTSZ);
                dfont.setColor( LeaderStyleReportUtils.sliceBaseColors[dr.getDominantTypeIndex()] );
                chk = new Chunk( lmsg_spec( LeaderStyleReportUtils.getCompetencyStub(dr.getDominantTypeIndex()) + ".name"), dfont);
                par.add( chk );
    ...
    protected void addCitationsSection() throws Exception
    {
        ...
        c = new PdfPCell( new Phrase( lmsg_spec("ls.Citations"), getHeaderFontXLarge() ));
        c.setBorder( Rectangle.NO_BORDER );
        c.setBackgroundColor( BaseColor.WHITE );
        c.setPaddingTop( 5 );
        setRunDirection( c );
    ...
    public void addInterpretationSection() throws Exception
    {
        try
        {
            List<String> dominantStyleStubs = getGroupDominantStyleStubs();
            List<String> missingStyleStubs = getGroupMissingStyleStubs();

            if( dominantStyleStubs.isEmpty() && missingStyleStubs.isEmpty() )
                return;
            ...
            Font styleNameFont = fontLargeBoldDarkBlue;
            Font contentFont = font;


            if( !dominantStyleStubs.isEmpty() )
            {
                y = addTitle( previousYLevel, lmsg_spec( dominantStyleStubs.size()==1 ? "grp.DomSty" : "grp.DomStys" ), null );
                y -= TPAD;

                for( String stub : dominantStyleStubs )
                {
                    c = new PdfPCell( new Phrase( lmsg_spec(  stub + ".name") + " (" + I18nUtils.getFormattedNumber(reportData.getLocale(), leaderStyleDataSet.getStylePercentages()[ LeaderStyleReportUtils.getIndexForStub(stub)], 1) + "%)", styleNameFont ) );
                    c.setBorder( Rectangle.NO_BORDER );
                    c.setHorizontalAlignment( Element.ALIGN_LEFT );
                    c.setVerticalAlignment( Element.ALIGN_TOP );
                    c.setBorderWidth( 0 );
                    ...
                    c = new PdfPCell( new Phrase( lmsg_spec(  "grp.dom." + stub ), contentFont ) );
                    c.setBorder( Rectangle.NO_BORDER );
                    c.setHorizontalAlignment( Element.ALIGN_LEFT );
                    c.setVerticalAlignment( Element.ALIGN_TOP );
                    c.setBorderWidth( 0 );
            ...
            if( !missingStyleStubs.isEmpty() )
            {
                y = addTitle( previousYLevel, lmsg_spec( missingStyleStubs.size()==1 ? "grp.GapSty" : "grp.GapStys" ), null );
                y -= TPAD;
                ...
                for( String stub : missingStyleStubs )
                {
                    c = new PdfPCell( new Phrase( lmsg_spec(  stub + ".name") + " (" + I18nUtils.getFormattedNumber(reportData.getLocale(), leaderStyleDataSet.getStylePercentages()[ LeaderStyleReportUtils.getIndexForStub(stub)], 1) + "%)", styleNameFont ) );
                    c.setBorder( Rectangle.NO_BORDER );
                    c.setHorizontalAlignment( Element.ALIGN_LEFT );
                    ...
                    c = new PdfPCell( new Phrase( lmsg_spec(  "grp.gap." + stub ), contentFont ) );
                    c.setBorder( Rectangle.NO_BORDER );
                    c.setHorizontalAlignment( Element.ALIGN_LEFT );
                    c.setVerticalAlignment( Element.ALIGN_TOP );
                    c.setBorderWidth( 0 );
        ...
        {
            LogService.logIt( e, "BaseLeaderStyleGroupReportTemplate.addInterpretationSection()" );
            throw e;
        }
    ...
    public List<String> getGroupDominantStyleStubs()
    {
        float maxPercent = 0;
        float nextHighestPercent = 0;
        int maxIndex = -1;
        int nextHighestIndex = -1;

        float[] percentages = leaderStyleDataSet.getStylePercentages();
        for( int i=0;i<5;i++ )
        {
            if( percentages[i]<=0 )
                continue;
            ...
            {
                nextHighestPercent = maxPercent;
                maxPercent = percentages[i];
                nextHighestIndex = maxIndex;
                maxIndex = i;
            }
        }

        List<String> out = new ArrayList<>();
        out.add( LeaderStyleReportUtils.getCompetencyStub(maxIndex) );

        if( nextHighestIndex>0 && nextHighestPercent>0 )
        {
            float pctOfMax = 100*nextHighestPercent/maxPercent;
            if( pctOfMax>75f )
                out.add( LeaderStyleReportUtils.getCompetencyStub(nextHighestIndex) );
        }

        return out;
    }

    public List<String> getGroupMissingStyleStubs()
    {
        float maxPercent=-1;
        float secondHighestPercent=-1;
        float minPercent = 101;
        float nextLowestPercent = 101;
        int minIndex = -1;
        ...
        for( int i=0;i<5;i++ )
        {
            ...
            if( percentages[i]>=maxPercent )
            {
                secondHighestPercent = maxPercent;
                maxPercent = percentages[i];
            }

            if( percentages[i]<=minPercent )
            {
                nextLowestPercent = minPercent;
                minPercent = percentages[i];
                nextLowestIndex = minIndex;
                minIndex = i;
            }
        }
        ...
        // see if we are too close to max
        float pctOfMax = 100*minPercent/maxPercent;

        // too close to max.
        if( pctOfMax>=80)
            return out;

        // If we used the second highest, see if too close
        float secondHighestPctOfMax = 100*secondHighestPercent/maxPercent;
        if( secondHighestPctOfMax>=75 )
        {
            float pctOfSecondHighest = 100*minPercent/maxPercent;

            // too close to max.
            if( pctOfSecondHighest>=80)
                return out;
        }

        out.add( LeaderStyleReportUtils.getCompetencyStub(minIndex) );

        if( nextLowestIndex>0 && nextLowestPercent>0 && nextLowestPercent<25f )
        {
            pctOfMax = 100*nextLowestPercent/maxPercent;

            // too close to max.
            if( pctOfMax>=80)
                return out;

            // too close to second highest
            if( secondHighestPctOfMax>=75 )
            {
                float pctOfSecondHighest = 100*nextLowestPercent/maxPercent;
            ...
            // too close to lowest
            float pctOfMin = 100*nextLowestPercent/minPercent;
            if( pctOfMin<175f )
                out.add( LeaderStyleReportUtils.getCompetencyStub(nextLowestIndex) );
        }
    ...
    public void addBlueBar() throws Exception
    {
        ...
        {
            LogService.logIt( e, "BaseLeaderStyleGroupReportTemplate.addBlueBar()" );
            throw e;
        }
    ...
    */
    public Map<String,Object[]> getGroupPercentageScoreValMap()
    {
        ...
        for( int i=0;i<leaderStyleDataSet.getStylePercentages().length; i++ )
        {
            //nameUppercase = LeaderStyleReportUtils.getCompetencyStubLetter(i).toUpperCase();
            //out.put( LeaderStyleReportUtils.getCompetencyStubLetter(i), new Object[]{ nameUppercase, Float.valueOf(discScoreVals[i])} );
            key = LeaderStyleReportUtils.getCompetencyStub(i) + ".name";
            out.put(LeaderStyleReportUtils.getCompetencyStub(i), new Object[]{ lmsg_spec(key), leaderStyleDataSet.getStylePercentages()[i], (int)leaderStyleDataSet.getStyleCounts()[i]} );

            if( DEBUG )
            {
                out.put( LeaderStyleReportUtils.getCompetencyStub(i), new Object[]{ lmsg_spec(key), (float)100f*(i+1)/10f , (i+1)} );
                // LogService.logIt( "BaseLeaderStyleGroupReportTemplate.getGroupPercentageScoreValMap() " + LeaderStyleReportUtils.getCompetencyStubLetter(i) + ": " + (float)100f*(i+1)/10f + ", count=" + (i+1));
            }
    ...
    public void addLeaderStyleStylesExplained() throws Exception
    {
        try
        {
            previousYLevel =  currentYLevel;

            float y = addTitle(previousYLevel, lmsg_spec( "ls.StylesExpl" ), null );
            ...
            setRunDirection( touter );
            touter.setTotalWidth( pageWidth - 2*CT2_MARGIN - 2*CT2_BOX_EXTRAMARGIN );
            touter.setWidths( new float[]{1.3f, 4f } );
            touter.setLockedWidth( true );
            // t.setHeaderRows( 1 );


            //BaseFont bf = fontBold.getBaseFont();
            Font styleNameFont = fontXLargeBoldDarkBlue; //  fontBold;
            Font styleDescFont = this.font;
            Font styleAttributeFont = this.fontItalic; //  fontBold;
            Font listItemFontBold = this.font;
            Font listItemFont = this.font;
            // String styleLetter;
            ...
            for( int i=0; i<5; i++ )
            {
                ...
                c = new PdfPCell( new Phrase( lmsg_spec(  LeaderStyleReportUtils.LEADER_STYLE_STUBS[i] + ".name" ), styleNameFont ) );
                c.setBorder( Rectangle.NO_BORDER );
                c.setColspan(2);
                c.setHorizontalAlignment( Element.ALIGN_LEFT );
                c.setVerticalAlignment( Element.ALIGN_TOP );
                ...
                c = new PdfPCell( new Phrase( lmsg_spec(  LeaderStyleReportUtils.LEADER_STYLE_STUBS[i] + ".description" ), styleDescFont ) );
                c.setColspan(2);
                c.setBorder( Rectangle.NO_BORDER );
                c.setHorizontalAlignment( Element.ALIGN_LEFT );
                c.setVerticalAlignment( Element.ALIGN_TOP );
                ...
                c = new PdfPCell( new Phrase( lmsg_spec( "ls.Strengths"), styleAttributeFont ) );
                c.setBorder( Rectangle.NO_BORDER );
                c.setHorizontalAlignment( Element.ALIGN_RIGHT );
                c.setVerticalAlignment( Element.ALIGN_TOP );
                c.setBorderWidth( 0 );
                ...
                c = new PdfPCell();
                itemListPairs = lmsg_spec_list_pairs(LeaderStyleReportUtils.LEADER_STYLE_STUBS[i] + ".strengths");
                addListItemPairGroupToTableCell(c, itemListPairs, listItemFontBold, listItemFont, 0);
                itemListPairs.clear();
                touter.addCell(c);

                c = new PdfPCell( new Phrase( lmsg_spec( "ls.AreasGrowth"), styleAttributeFont ) );
                c.setBorder( Rectangle.NO_BORDER );
                c.setHorizontalAlignment( Element.ALIGN_RIGHT );
                c.setVerticalAlignment( Element.ALIGN_TOP );
                c.setBorderWidth( 0 );
                ...
                c = new PdfPCell();
                itemListPairs = lmsg_spec_list_pairs(LeaderStyleReportUtils.LEADER_STYLE_STUBS[i] + ".growth");
                addListItemPairGroupToTableCell(c, itemListPairs, listItemFontBold, listItemFont, 0);
                itemListPairs.clear();
                touter.addCell(c);

                c = new PdfPCell( new Phrase( lmsg_spec( "ls.WhenWorks"), styleAttributeFont ) );
                c.setBorder( Rectangle.NO_BORDER );
                c.setHorizontalAlignment( Element.ALIGN_RIGHT );
                c.setVerticalAlignment( Element.ALIGN_TOP );
                c.setBorderWidth( 0 );
                ...
                c = new PdfPCell();
                itemList = lmsg_spec_list(LeaderStyleReportUtils.LEADER_STYLE_STUBS[i] + ".whenworks");
                addListItemGroupToTableCell(c, itemList, listItemFont, 0);
                itemList.clear();
                touter.addCell(c);

                c = new PdfPCell( new Phrase( lmsg_spec( "ls.NotWorks"), styleAttributeFont ) );
                c.setBorder( Rectangle.NO_BORDER );
                c.setHorizontalAlignment( Element.ALIGN_RIGHT );
                c.setVerticalAlignment( Element.ALIGN_TOP );
                c.setBorderWidth( 0 );
                ...
                c = new PdfPCell();
                itemList = lmsg_spec_list(LeaderStyleReportUtils.LEADER_STYLE_STUBS[i] + ".notworks");
                addListItemGroupToTableCell(c, itemList, listItemFont, 0);
                itemList.clear();
                touter.addCell(c);
                ...
                if( currentYLevel>150 )
                    addFooterBar( lmsg_spec( "ls.StylesExpl").toUpperCase(), i%2==0, fontXLargeBoldWhite );
        ...
        {
            LogService.logIt( e, "BaseLeaderStyleGroupReportTemplate.addLeaderStylesExplained() " + reportData.toString() );
            throw e;
        }
    }
    ...
    public void addPurposeOfAssessment() throws Exception
    {
        ...
        {
            previousYLevel =  currentYLevel;
            float y = addTitle(previousYLevel, lmsg_spec( "ls.Purpose" ), null );
            y -= TPAD;

            BaseFont bf = fontLargeBold.getBaseFont();
            Font listHeaderFont = new Font(bf, FONTSZ);
            listHeaderFont.setColor( new BaseColor( 0x0b, 0x50, 0x8b) );
            Font listItemFont = font;
            Font listItemFontBold = fontBold;
    ...
    public void setLeaderStyleDataSet(LeaderStyleGroupDataSet discDataSet)
    {
        this.leaderStyleDataSet = discDataSet;
    }
    ...
    public void addFooterBar( String title, boolean includeImage, Font fnt ) throws Exception
    {
            ...
            if( !includeImage )
            {
                ...
                // LogService.logIt( "BaseLeaderStyleGroupReportTemplate.addFooterBar() footerImageIndex=" + footerImageIndex + ", footerImageUri=" + footerImageUri + ", footerImage=" + (footerImage!=null) );
                if( footerImage !=null )
                {
    ...
    public void addListItemGroupToTableCell( PdfPCell c, List<String> itemList, Font listItemFont, int extraLeftPadding) throws Exception
    {
        com.itextpdf.text.List cl = new com.itextpdf.text.List( com.itextpdf.text.List.UNORDERED, 12 );
        cl.setListSymbol( "\u2022");
        cl.setIndentationLeft( 10 );
    ...
    public void addListItemPairGroupToTableCell( PdfPCell c, List<String[]> itemListPairs, Font listItemFontBold, Font listItemFont, int extraLeftPadding) throws Exception
    {
        com.itextpdf.text.List cl = new com.itextpdf.text.List( com.itextpdf.text.List.UNORDERED, 12 );
        cl.setListSymbol( "\u2022");
        cl.setIndentationLeft( 10 );
   ...
   public float addTitleLarge( float startY, String title, Font fnt ) throws Exception
    ...
    public float addTitleLargeRTL( float startY, String title, Font fnt ) throws Exception
    {
        ...
        {
            LogService.logIt( e, "BaseLeaderStyleGroupReportTemplate.addTitleLargeRTLLarge()" );
            throw new STException( e );
        }
    }
    ...
    public float getTitleHeightLarge( String title, Font fnt ) throws Exception
    {
        ...
        {
            LogService.logIt( e, "BaseLeaderStyleGroupReportTemplate.getTitleHeightLarge()" );
            throw new STException( e );
        }
    }
    ...
    public void addNotesSection() throws Exception
    {
        ...
        addTitle( currentYLevel , lmsg("g.Notes"), lmsg( "g.NotesSubtitle" ) );
    }
