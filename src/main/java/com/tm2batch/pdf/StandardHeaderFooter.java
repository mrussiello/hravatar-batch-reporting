...
public class StandardHeaderFooter extends PdfPageEventHelper
{
    ...
    public StandardHeaderFooter( Document d , Locale l, String t, ReportData rd, ReportSettings rs ) throws Exception
    {
        ...
        copyright = MessageFactory.getStringMessage(locale, "g.copyrightforreport", new String[]{ Integer.toString( c.get( Calendar.YEAR) ) } );
    }
    ...
    public void init( PdfWriter w ) throws Exception
    {
        if( HEADER_HEIGHT == 0 )
        {
            ...
            Image logo = reportSettings.getLogoWhiteTextSmall();
            
            HEADER_HEIGHT = Math.max( logo==null ? 0 : logo.getScaledHeight(), HEADER_TITLE_HEIGHT ) + 10;

            HEADER_IMAGE_X = 10;
            HEADER_IMAGE_Y = pageHeight - ((HEADER_HEIGHT - (logo==null ? 0 : logo.getScaledHeight()))/2 ) - (logo==null ? 0 : logo.getScaledHeight());
    ...
    private void createHeader( PdfWriter w, Document d, boolean dark )
    {
        try
        {
            ...
            Image logo = dark ? reportSettings.getLogoWhiteTextSmall() : reportSettings.getLogoDarkTextSmall();
            // Add the logo on top of the background in lower layer
            if( logo != null )
                ITextUtils.addDirectImage( w, logo , HEADER_IMAGE_X, HEADER_IMAGE_Y, true );
        }
